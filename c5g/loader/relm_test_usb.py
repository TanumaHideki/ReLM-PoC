    ...
    Thread[
        ...
        Do()[
            Do()[
                ...
                If(usb.rcode(usb.GetConfDescr(1, 0, 18, 0, fifo.port)) != 0)[
                    usb.usb_task_state(usb.USB_STATE_ERROR),
                    Continue(),
                ],
                serial.Print("\tDescriptor length: 0x").Hex(fifo.Pop()).Println(),
                serial.Print("\tDescriptor type: 0x").Hex(fifo.Pop()).Println(),
                serial.Print("\tTotal length: 0x")
                .Hex(fifo.Pop() + fifo.Pop() * 0x100)
                .Println(),
                serial.Print("\tNumber of interfaces: 0x").Hex(fifo.Pop()).Println(),
                serial.Print("\tConfiguration value: 0x").Hex(fifo.Pop()).Println(),
                serial.Print("\tConfiguration string: "),
                If(usb.rcode(usb.GetString(1, fifo.Pop(), 0x0409, fifo_str.port)) != 0)[
                    usb.usb_task_state(usb.USB_STATE_ERROR),
                    Continue(),
                ],
                While(~fifo_str.IsEmpty())[serial.fifo_send.Push(fifo_str.Pop()),],
                serial.Println(),
                serial.Print("\tnAttributes: 0x").Hex(fifo.Pop()).Println(),
                serial.Print("\tnMax.power: 0x").Hex(fifo.Pop()).Println(),
                serial.Println("Interface descriptor:"),
                serial.Print("\tDescriptor length: 0x").Hex(fifo.Pop()).Println(),
                serial.Print("\tDescriptor type: 0x").Hex(fifo.Pop()).Println(),
                serial.Print("\tInterface number: 0x").Hex(fifo.Pop()).Println(),
                serial.Print("\tAlternate setting: 0x").Hex(fifo.Pop()).Println(),
                serial.Print("\tNumber of endpoints: 0x").Hex(fifo.Pop()).Println(),
                serial.Print("\tInterface class: 0x").Hex(fifo.Pop()).Println(),
                serial.Print("\tInterface sub-class: 0x").Hex(fifo.Pop()).Println(),
                serial.Print("\tInterface protocol: 0x").Hex(fifo.Pop()).Println(),
                serial.Print("\tInterface string: "),
                If(usb.rcode(usb.GetString(1, fifo.Pop(), 0x0409, fifo_str.port)) != 0)[
                    usb.usb_task_state(usb.USB_STATE_ERROR),
                    Continue(),
                ],
