...
アキュムレータ（Acc）には1.0～2.0に正規化された仮数部が残りますので、この逆数を計算して先程の指数部の除算結果と掛けた上で最終的な除算結果を求める必要があります。
...
誤差を評価するため $xy-1$ を $ulp=2^{-23}$ 単位で表示したものが右端になりますが、一部の点で -0.5ulp か +1.0ulp の誤差がある以外は誤差がほぼ０となっています。（負方向の誤差は指数が変わるので0.5ulp単位）
