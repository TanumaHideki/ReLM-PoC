...
現状の整数除算は分子分母ともに符号なし32ビット整数として扱う処理となっていますので、負数をそのまま入力すると異常な結果となります。

もし負数を扱いたい場合は、まず正数に直してから除算を行い、後で符号を反映する処理を実施してください。

Playable PoCのバブルエステートは資産状況を表す棒グラフ表示のスケーリングに除算を使用していますので、そのまま除算命令のテストコードにもなっています。

## 浮動小数点演算命令拡張

浮動小数点演算に関しては、以下の表に示す通り、比較的多くの拡張命令を使用しています。
...
* FMUL, FSQU  
  FMULは乗算、FSQUは平方を計算。  
  ITOFXを後処理として利用し、FMUL, ITOFXの２命令で浮動小数点の乗算を実行。
* FADD, FADDX  
  後処理も含めてFADD, FADDX, ITOF, ITOFXの４命令で浮動小数点の加算を実行。  
  正 + 正、負 + 負のように符号が等しい数同士の加算では、正規化が不要となるのでITOFを省略した３命令でも構わない。
* ROUND/TRUNC  
