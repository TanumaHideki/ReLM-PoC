...
ただし、現状の整数除算は分子分母ともに符号なし32ビット整数として扱う処理となっていますので、負数をそのまま入力すると異常な結果となります。
...
* FMUL, FSQU  
  FMULは乗算、FSQUは平方を計算。  
  ITOFXを後処理として利用し、**FMUL, ITOFX** の２命令で浮動小数点の乗算を実行。
* FADD, FADDX  
  後処理も含めて **FADD, FADDX, ITOF, ITOFX** の４命令で浮動小数点の加算を実行。  
  正 + 正、負 + 負のように符号が等しい数同士の加算では正規化が不要となるので、**FADD, FADDX, ITOFX** と命令数を削減できる。
* PUTM, PUTMX  
  引き算の専用命令は特に無いため、符号反転した上で加算を実行する。  
  **XOR 0x80000000** の１命令で符号反転は可能だが、なるべく命令数を減らすため、PUTと同時に符号反転を実施する命令 **PUTM** が用意されている。  
  実際はPUTMとPUTSは同じ命令コードが割り当てられており、オペランドの命令アドレスの最上位ビットを立てることで区別している。  
  実装の都合上、OPB形式の場合は命令コードの最上位ビットを立てた **PUTMX** を使用する必要がある。  
  特に著しい回路規模の増大や性能低下があるとは考えられないため、実際には基本命令セットの回路にPUTM, PUTMXの機能が未公開命令として組み込まれている。
* ROUND/TRUNC  
  加算と合わせて様々なモードの整数への丸め（round, floor, ceil, trunc）を実現。  
  例えばROUND 1.0（オペランドはIEEE754のbinary32形式）とすると
* FTOI  
  浮動小数点を整数に変換。
* FCOMP  
  浮動小数点の大小を比較。
