...
整数除算アルゴリズムは内部的に２ワード（+1 ビット）分のカスタムレジスタを使用します。

これらをC, Dレジスタとして除算アルゴリズム実行時のデータの流れを以下に示します。

| OpCode | D | C | B | A | X (operand) or #Remarks |
| --- | --- | --- | --- | --- | --- |
| DIV | | | | **N (numerator)** | **D (denominator)** |
| OPB DIVLOOP | 3D>>1 | D | r0 | N<<2 + q0 | DIVLOOP (opcode) |
| OPB DIVLOOP | 3D>>1 | D | r0<<3 + r1 | N<<5 + q0<<3 + q1 | DIVLOOP (opcode) |
| OPB DIVLOOP | | | . . . | | DIVLOOP (opcode) |
| | | | **R (remainder)** | **Q (quotient)** | # After DIVLOOP x 10 times |
...
先読み比較の際、除数および除数の３倍の値を高速に参照する必要があるため、DIV命令でカスタムレジスタ（C, D）に格納して以後のDIVLOOP命令の実行を支援します。

DIV命令では予め２ビット分の処理を行い、後のDIVLOOPが10回分で30ビット処理できますので、合わせて32ビットの商を計算して整数除算の処理が完了します。

計11命令で商、剰余の場合はもう１命令掛かりますので、50MHzクロックで220ns, 240nsで整数除算および剰余の計算が可能になります。
...
他のレジスタは仮数部のみの除算を実行するために初期化されますが、実際の除算は整数除算用の **DIVLOOP** 命令を利用して３ビットずつ処理されます。
...
このコンパイル結果のダンプ出力は以下の様になり、FDIV命令も含めると15命令（50MHzクロックで300ns）で浮動小数点除算が実行可能です。

~~~
9B9B:   FDIV    +1.000000E+00           50:     y := Float(1.0 / x),
9B9C:   PUT     9BA8:                   50:     ->      y := Float(1.0 / x),
...
9BA6:   OPB     FDIVLOOP                50:     y := Float(1.0 / x),
9BA7:   ITOF    +1.600000E+01           50:     y := Float(1.0 / x),
9BA8:   FMUL    00000000                50:     y := Float(1.0 / x),
9BA9:   OPB     ITOF                    50:     y := Float(1.0 / x),
~~~

DIVLOOP命令の後のFDIVLOOP命令は浮動小数点除算用にDIVLOOP命令の動作を少し変更したもので、３ビットの商のうち最後のビットを丸め処理用の[stickyビット](https://en.wikipedia.org/wiki/Floating-point_arithmetic#Addition_and_subtraction)に変更したものです。
...
また、もしDIVLOOP命令を１つ減らした場合、以下の様にさらに大きな誤差が出るようになりますので、DIVLOOP命令８回とその後にFDIVLOOP命令が、単精度浮動小数点除算に最低限必要な処理となることがわかります。
