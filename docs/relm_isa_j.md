...
ただしここで、RHS で最初にアキュムレータを変更する命令がLOADとなる（コンパイラ出力では大体そうなる）場合に、これをBLOADに置き換えたものを RHS' とします。（もしそうならない場合、１命令増えるが RHS の前に OPB BLOADX を置く）
...
以下の拡張では、１ワード分のカスタムレジスタ（C）を割り当てています。

* 整数除算拡張（[relm_custom_div.v](../relm_custom_div.v)）
* 浮動小数点演算および整数除算拡張（[relm_custom_div_fp.v](../relm_custom_div_fp.v)）
